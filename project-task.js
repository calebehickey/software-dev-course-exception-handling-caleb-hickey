...
function addAnimal(name, fee) {
    try {
        ...
        if (fee < 0) {
            throw new Error("Fee cannot be a negative")
        }
        animals.push(name)
        fees.push(fee)
        return true
            ...
            console.log("There was an error with what you entered, please try again."); 
            return false
        }
    }
...
function getAdoptionFee(animalName) {
    ...
    try {
        ...
        return fees[index];
            ...
            console.log("No animal by that name was found");
            return false
        }
...
while (true) {
    ...
    if (action === "add") {
        ...
        let result = addAnimal(animal, fee);
        if (result === true) {
            console.log(`${animal} added with a fee of $${fee}.`);
    } else if (action === "fee") {
        let animal = readlineSync.question("Enter the animal's name to find its adoption fee: ");
        let (result = getAdoptionFee(animalName))
        if (result != true) {
